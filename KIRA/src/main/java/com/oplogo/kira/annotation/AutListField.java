...
public class AutListField {
    ...
    public static AutListField create(Field field) {
        AutListField r = null;
        Display display = field.getAnnotation(Display.class);
        if (display == null) {
            r = new AutListField(field);
        } else {
            if (display.enabled() && display.scope() <= Display.Scope.LIST) {
                r = new AutListField(field);
            }
        }
        return r;
    ...
    private AutListField(Field field) {
        Display display = field.getAnnotation(Display.class);
        this.field = field;
